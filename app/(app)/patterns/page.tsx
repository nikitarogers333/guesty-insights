          ...
          {dayData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={dayData}>
                <XAxis dataKey="day" />
                <YAxis />
                <Tooltip labelFormatter={(label, payload) => payload?.[0]?.payload?.fullDay || label} />
          ...
          {dayData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart data={dayData}>
                ...
                <Radar name="Bookings" dataKey="bookings" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.5} />
                <Tooltip />
              </RadarChart>
            </ResponsiveContainer>
          ) : (
            ...
            <>
              <div className="mb-4 p-4 bg-slate-50 rounded-lg">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-2xl font-bold text-slate-900">{cancellations.total_cancellations}</p>
              ...
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => (name === "rate" ? `${value.toFixed(1)}%` : value)}
                  />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {(() => {
              ...
              const advance = leadTime.buckets
                ...
                .reduce((sum, b) => sum + b.count, 0);
              const maxBucket = leadTime.buckets.reduce(
